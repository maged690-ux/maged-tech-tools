    ...
    try {
        ...
        if (fileId) {
            ...
            if (fileData.ok) {
                ...
                // 🔥 حقنة الذكاء البصري الصارمة (لمنع التأليف) 🔥
                if (mimeType.includes('image')) {
                    const smartVisionPrompt = `أنت الآن في وضع الفحص البصري الدقيق كمهندس صيانة.
                    1. إذا كانت الصورة لكرتونة هاتف: استخرج (IMEI, السيريال، الموديل).
                    تحذير هام جداً: إياك أن تخمن أو تؤلف أي أرقام. إذا لم تكن متأكداً من الرقم بنسبة 100% أو كانت الصورة غير واضحة، قل صراحة: "يا معلم الصورة مغبشة وغير واضحة، ما قدرت اقرأ الأرقام بدقة، صورها أوضح أو ابعتها كملف".
                    2. إذا كانت للوحة أم (Motherboard): ابحث عن أي آثار حرق، أكسدة (ماء)، أو مكونات مفقودة، واشرح المشكلة بالتفصيل.
                    ${text ? 'السؤال المرفق من الفني: ' + text : 'حلل هذه الصورة وأعطني التفاصيل.'}`;
